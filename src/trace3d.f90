...
!for check limiter
  ...
  SELECT CASE(TRIM(def_start))
    ...
    CASE DEFAULT
      ...
      PRINT fmt, f0(1,1), f0(mr,1), f0(1,2), f0(mr,2), f0(1,3) * 180 / pi

      PRINT *
      PRINT *, '   dRflx [m]   dZflx [m]   dPflx [deg] '
      PRINT *, '---------------------------------------'

      PRINT fmt, drflx, dzflx, dpflx * 180 / pi
  ...
  loop0050 : DO js=0,mr
    ...
    !CALL odeint(x0, f0(js,:), ln, h, lnx, dopri5, subf3d, f(1,js,:,:), icount(1,js), iout(1,js))
    CALL odeadm5(x0, f0(js,:), ln, h, lnx, dopri5, subf3d, f(1,js,:,:), icount(1,js), iout(1,js))
    !IF(lupdown) CALL odeint(x0, f0(js,:), ln, -h, lnx, dopri5, subf3d, f(2,js,:,:), icount(2,js), iout(2,js))
    IF(lupdown) CALL odeadm5(x0, f0(js,:), ln, -h, lnx, dopri5, subf3d, f(2,js,:,:), icount(2,js), iout(2,js))
  ...
  IF(llimiter .OR. lcheck_vessel)THEN
    ...
    DO js=0,mr
      ...
      END IF
#ifdef LCOUT
      WRITE(52,'(3(ES22.10,A))') f0(js,1), ',', f0(js,2), ',', ABS(h) * (icount(1,js) + icount(2,js)), ','
#endif
    END DO
  ...
  loop_poloidal: DO l=1,16
    ...
    loop_radial: DO js=0,mr
      ...
      END IF
      !IF(p1 == pcros(l))THEN
      !  WRITE(50,'(3(ES15.7,A1))') r1, ',', z1, ',', ABS(h) * icount(1,js), ','
      ...
      loop0110: DO i=2,icount(1,js)
        ...
        IF(w0 * w1 < 0.0_DP)THEN
          ...
          rq =  er * ep + r0
          zq =  ez * ep + z0
          WRITE(50,'(3(ES15.7,A1))') rq, ',', zq, ',', ABS(h) * icount(1,js), ','
      ...
      if_lupdown: IF(lupdown)THEN
        ...
        END IF
        !IF(p1 == pcros(l))THEN
        !  WRITE(50,'(3(ES15.7,A1))') r1, ',', z1, ',', ABS(h) * icount(2,js), ','
        !  IF(lfigout) CALL plot_poincare(r1, z1)
        !END IF
        loop0120: DO i=2,icount(2,js)
          ...
          IF(w0 * w1 < 0.0_DP)THEN
            ...
            rq =  er * ep + r0
            zq =  ez * ep + z0
            WRITE(50,'(3(ES15.7,A1))') rq, ',', zq, ',', ABS(h) * icount(2,js), ','
            IF(lfigout) CALL plot_poincare(rq, zq)
          END IF
        END DO loop0120
      END IF if_lupdown
  ...
  DO js=0,mr
    ...
    p1 =  f0(js,3)
    IF(z1 == zcros)THEN
      WRITE(51,'(3(ES15.7,A1))') r1 * COS(p1), ',', r1 * SIN(p1), ',', ABS(h) * icount(1,js), ','
    END IF
    ...
    DO i=2,icount(1,js)
      ...
      IF(w0 * w1 < 0.0_DP)THEN
        ...
        rq =  er * ez + r0
        pq =  ep * ez + p0
        WRITE(51,'(3(ES15.7,A1))') rq * COS(pq), ',', rq * SIN(pq), ',', ABS(h) * icount(1,js), ','
      END IF
    END DO
    IF(lupdown)THEN
      r1 =  f0(js,1)
      ...
      IF(z1 == 0.0_DP)THEN
        WRITE(51,'(3(ES15.7,A1))') r1 * COS(p1), ',', r1 * SIN(p1), ',', ABS(h) * icount(2,js), ','
      END IF
      DO i=2,icount(2,js)
        r0 =  r1
        z0 =  z1
        ...
        IF(w0 * w1 < 0.0_DP)THEN
          ...
          rq =  er * ez + r0
          pq =  ep * ez + p0
          WRITE(51,'(3(ES15.7,A1))') rq * COS(pq), ',', rq * SIN(pq), ',', ABS(h) * icount(2,js), ','
        END IF
      END DO
    END IF
  END DO
